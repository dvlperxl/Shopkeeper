...
<UITableViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,VillageChooseViewDelegate>
...
- (void)httpRequestAddMember:(NSString*)name
                     address:(NSString*)address
                     village:(NSString *)village
                      mobile:(NSString*)mobile
{
    [KKProgressHUD showMBProgressAddTo:self.view];
    [[APIService share]httpRequestAddMemberWithAddress:address
                                            customerId:nil
                                                areaId:self.areaId
                                           customerNme:name
...
- (void)onSaveButtonAction
{
    ...
    NSString *mobile = [param objectForKey:@"mobile"];
    
    if (storeName == nil &&storeName.length<1)
    {
    ...
    if (storeName.length>16) {
        ...
        [[KKToast makeToast:@"姓名最多输入16个字"] show];
        return;
    }
    ...
    {
        [[KKToast makeToast:@"手机号码"] show];
        return;
    }
    
    if (![mobile validateMobile]) {
        
        [[KKToast makeToast:@"手机号码格式不正确"] show];
        return;
    }
    ...
    [self httpRequestAddMember:storeName
                       address:address
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    KKCellModel *cellModel = [self.tableViewModel cellModelAtIndexPath:indexPath];
    if ([cellModel.cellType isEqualToString:@"所在地区"])
    {
        ...
        [param setObject:@"AddMemberViewController" forKey:@"backClassName"];
...
-(KKTableViewModel *)tableViewModel
{
    if (!_tableViewModel)
    {
        ...
        for (NSInteger i = 0;i<titles.count;i++)
        {
            ...
            if ([data.title isEqualToString:@"手机号码"]) {
                data.contentKey = @"mobile";
