//
//  AddReceivePeopleViewController.m
//  Shopkeeper
//
...
#import "AddReceivePeopleViewController.h"
...
@interface AddReceivePeopleViewController ()<UITableViewDelegate,VillageChooseViewDelegate>
...
@implementation AddReceivePeopleViewController
...
- (void)viewDidLoad {
    ...
    if (self.addressId)
    {
        self.tableViewModel = [AddReceivePeopleModel tableViewModelWithAddressDetail:self.address];
        self.areaId = self.address[@"areaId"];
        self.village = self.address[@"village"];
        self.tableView.tableViewModel = self.tableViewModel;
        
    }else
    {
        self.tableViewModel = [AddReceivePeopleModel tableViewModelWithAddressDetail:nil];
        self.tableView.tableViewModel = self.tableViewModel;
    }
...
- (void)httpRequestAddReceivePersonAddress:(NSDictionary*)param
{
    [KKProgressHUD showMBProgressAddTo:self.view];
    
    [[APIService share]httpRequestModifyReceivePersonAddress:self.storeId
                                                storeAddress:param
                                                     success:^(NSDictionary *responseObject) {
...
{
    NSMutableDictionary *param = [AddReceivePeopleModel getAddReceivePeopleParam:self.tableViewModel].mutableCopy;
    
    NSString *contactName = param[@"receivePerson"];
    
    if (contactName.length<1) {
        
        [[KKToast makeToast:@"姓名不能为空"]show];
        return;
    }
    
    if (contactName.length>16)
    {
        [[KKToast makeToast:@"姓名最多16个字"]show];
        return;
    }
    
    if ([contactName firstLetterLegal])
    {
        [[KKToast makeToast:@"姓名首字不能为特殊符号"]show];
        return;
    }
    
    if ([contactName stringContainsEmoji])
    {
        [[KKToast makeToast:@"请输入正确的姓名"]show];

        return;
    }
    
    
    NSString *contactPhone = param[@"receivePhone"];
    ...
    {
        [[KKToast makeToast:@"详细地址不能为空"]show];
        return;
    }
    ...
    if ([address stringContainsEmoji])
    {
        [[KKToast makeToast:@"请输入正确的地址"]show];
        return;
    }
    
    NSString *area = param[@"areaId"];
    
    [param setObject:address forKey:@"addressDetail"];
    ...
    NSString *fullAddress = [NSString stringWithFormat:@"%@ %@ %@",area,_village,address];
    [param setObject:fullAddress forKey:@"fullAddress"];
    [self httpRequestAddReceivePersonAddress:param];
}
...
- (void)villageChooseViewDidChooseVillage:(NSString *)village
{
    ...
    NSArray *indexPaths =  [self.tableViewModel findCellModelIndexPathsWithCellType:@"village"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cellModel.cellType isEqualToString:@"areaId"])
    {
        ...
        [param setObject:self.areaId forKey:@"areaId"];
        [param setObject:@"AddReceivePeopleViewController" forKey:@"backClassName"];
...
- (void)setCallBack:(NSDictionary *)callBack
{
    if (callBack[@"id"])
    {
        ...
        NSArray *indexPaths1 =  [self.tableViewModel findCellModelIndexPathsWithCellType:@"village"];
        ...
        NSArray *indexPaths =  [self.tableViewModel findCellModelIndexPathsWithCellType:@"areaId"];
        NSIndexPath *indexPath = indexPaths.firstObject;
