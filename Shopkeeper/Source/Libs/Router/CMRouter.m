...
- (void)presentControllerWithControllerName:(NSString *)viewControllerName param:(NSDictionary *)param
{
    ...
    NSMutableDictionary *mp = param.mutableCopy;
    [mp setObject:@(1) forKey:@"present"];

    [self showViewController:viewControllerName param:mp];
}
...
- (void)presentControllerWithRouterModel:(RouterModel*)model
{
    ...
    NSMutableDictionary *mp = model.param.mutableCopy;
    [mp setObject:@(1) forKey:@"present"];
    [self presentControllerWithControllerName:model.className param:mp.copy];
}
...
//    NSMutableDictionary *mp = param.mutableCopy;
//    [mp setObject:@(1) forKey:@"present"];
//    [self pressntVController:controller parameters:mp.copy animated:YES];
//}
...
- (id)getObjectWithClassName:(NSString *)className
{
    ...
    if (vc == nil) {
        ...
        NSString *cls = [NSString stringWithFormat:@"%@.%@",bundleName,className];
        vc = [NSClassFromString(cls) new];
...
{
    NSRange range = NSRangeFromString(timer.userInfo);
    UINavigationController *nvc = [self currentNavigationController];
    if ([nvc isKindOfClass:[UINavigationController class]])
    {
        if (nvc.viewControllers.count>=range.location+range.length)
        {
            NSMutableArray *array = nvc.viewControllers.mutableCopy;
...
{
    NSString *valueStr = NSStringFromRange(range);
    NSTimer *timer = [NSTimer timerWithTimeInterval:0.2 target:self selector:@selector(handleHideTimer:) userInfo:valueStr repeats:NO];
...
- (void)setObject:(UIViewController*)viewController parameters:(NSDictionary *)parameters
{
    ...
    while (![NSStringFromClass(cls) hasPrefix:@"UI"])
    {
        if ([parameters isKindOfClass:[NSDictionary class]])
        {
            NSArray *properties = [self getAllPropertiesWithObject:cls];
