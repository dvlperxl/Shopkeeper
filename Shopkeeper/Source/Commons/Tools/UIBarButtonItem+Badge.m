...
- (void)updateBadgeFrame
{
    ...
    self.badge.bounds = CGRectMake(0, 0, minWidth + padding, minHeight + padding);
    CGFloat defaultOriginX = 0;
    UIView *superview = nil;
    if (self.customView) {
        superview = self.customView;
        defaultOriginX = superview.frame.size.width - self.badge.bounds.size.width/2;
        // Avoids badge to be clipped when animating its scale
        superview.clipsToBounds = NO;
    } else if ([self respondsToSelector:@selector(view)] && [(id)self view]) {
        superview = [(id)self view];
        defaultOriginX = superview.frame.size.width - self.badge.bounds.size.width;
    }
    self.badge.frame = CGRectMake(defaultOriginX, self.badgeOriginY, minWidth + padding, minHeight + padding);
